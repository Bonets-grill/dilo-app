...
export default function ShareMenu({ text, onClose, y }: ShareMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm" role="presentation" onClick={onClose} onTouchEnd={onClose}>
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
          ...
          <button type="button" onClick={onClose} aria-label={t("close") ?? "Close"} className="p-0.5 rounded-full active:bg-white/10">
            <X size={16} className="text-[#8e8e93]" aria-hidden="true" />
          </button>
        </div>
        ...
        {/* WhatsApp */}
        <button type="button" onClick={shareWhatsApp} className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:bg-white/10 border-b border-white/5">
        ...
        {/* Telegram */}
        <button type="button" onClick={shareTelegram} className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:bg-white/10 border-b border-white/5">
        ...
        {/* Copy */}
        <button type="button" onClick={copyText} className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:bg-white/10">
