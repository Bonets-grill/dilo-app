...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (authError) {
        // If Supabase auth fails, bypass to chat for demo
        console.warn("Auth error, bypassing:", authError.message);
        router.push("/chat");
        return;
      }
      ...
      <main className="flex items-center justify-center min-h-dvh px-6">
        <div className="text-center max-w-sm">
          ...
          <button
            ...
            className="mt-6 text-sm text-[#888] underline"
          >
            Skip for now →
          </button>
        </div>
    ...
    <main className="flex items-center justify-center min-h-dvh px-6">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full px-4 py-3 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a] text-sm font-medium hover:bg-[#222] transition flex items-center justify-center gap-2"
        >
